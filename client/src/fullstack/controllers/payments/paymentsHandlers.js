...
import { getPayunitClient } from "../../config/payunit.js";
...
const PAYUNIT_NOTIFY_URL =
  process.env.PAYUNIT_NOTIFY_URL?.trim() ||
  (BACKEND_URL
    ? `${BACKEND_URL}/api/next/payments/webhook`
    : `${FRONTEND_URL}/api/next/payments/webhook`);
...
async function initiatePayunitPayment({
  ...
  const client = getPayunitClient();
  if (!client) {
    const err = new Error("Payunit is not configured.");
    err.statusCode = 503;
    throw err;
  }
...
async function handlePaymentRequest(req, res, channel, customerType) {
  ...
  try {
    ...
    const notifyUrl = PAYUNIT_NOTIFY_URL;
...
export async function paymentsWebhook(req, res) {
  ...
  try {
    const payunit = getPayunitClient();
    if (transactionId && payunit) {
      const statusResponse = await payunit.collections.getTransactionStatus(
        transactionId
      );
...
export async function paymentsGetTransactionStatus(req, res) {
  try {
    const client = getPayunitClient();
    if (!client) {
      return res.status(503).json({ error: "Payunit is not configured." });
    }
